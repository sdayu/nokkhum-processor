...
int main(int argc, char** argv) {
	...
	const std::string default_log_directory = "/tmp/nokkhum-log/processors/";
	...
	program_desc.add_options()
        ...
        ("processor_id", boost::program_options::value<std::string>(), "set processor id")
        ;
    boost::program_options::positional_options_description pod;
    pod.add("processor_id", -1);
    ...
    if (!vm.count("processor_id")) {
    	std::cerr << "required processor_id" << std::endl;
    	std::cout << program_desc << std::endl;
    	return 1;
    }
	...
	if (*(log_directory.rbegin()) == '/'){
		log_directory = log_directory + vm["processor_id"].as<std::string>() + "/";
	}
	else	{
		log_directory = log_directory + "/" + vm["processor_id"].as<std::string>() + "/";
	}
	...
	std::string attribute = vm["processor_id"].as<std::string>();
